...
(function(window) {
  ...
  const ROLE_DEFAULTS = {
    ...
    'operations_manager': ['admin-v2', 'master-cache', 'launch-campaign', 'execution-plan', 'execution-entry', 'rates-master', 'dashboard', 'printing', 'stitching', 'delivery', 'payments', 'outstanding', 'report-daily', 'report-team', 'report-campaign', 'ppt', 'manual-upload', 'design-tasks'],
    'ops_manager': ['admin-v2', 'master-cache', 'launch-campaign', 'execution-plan', 'execution-entry', 'rates-master', 'dashboard', 'printing', 'stitching', 'delivery', 'payments', 'outstanding', 'report-daily', 'report-team', 'report-campaign', 'ppt'],
    'operations': ['execution-entry', 'launch-campaign', 'dashboard'],
    'execution': ['execution-entry', 'execution-plan'],
    'designer': ['designer-uploads', 'design-tasks', 'campaign-status', 'dashboard'],
    'design': ['designer-uploads', 'design-tasks', 'campaign-status', 'launch-campaign', 'dashboard'],
    'field': ['field'],
    'field_worker': ['field'],
    'printing': ['printing'],
    'printer': ['printing'],
    ...
    'driver': ['delivery'],
    'verifier': ['verifier'],
    'accountant': ['dashboard', 'payments', 'outstanding', 'report-campaign'],
    'data_manager': ['dashboard', 'master-cache', 'report-daily', 'report-team', 'report-campaign', 'ppt'],
    'data_entry': ['admin-v2'],
    'deo': ['admin-v2']
  };
  ...
  const MODULE_PAGES = {
    ...
    'dashboard': '/dashboard.html', 'printing': '/printing-v2.html',
    'stitching': '/stitching-v3.html', 'delivery': '/delivery.html',
  ...
  const AccessGuard = {
    ...
    // Validate whether the current user may open a given page path.
    // Used by login.html to avoid bouncing a user back to a page their role
    // can't access (e.g. printing operator → assign-operators.html).
    canAccessPage(pageUrl) {
      ...
      // Normalize: strip query/hash, leading slash, .html
      let path = String(pageUrl).split('?')[0].split('#')[0].replace(/^\//, '').replace(/\.html$/, '');

      // Pages everyone-logged-in may open (shells / neutral)
      const ALWAYS_OK = ['app', 'index', 'login', 'hub'];
      if (ALWAYS_OK.indexOf(path) !== -1) return true;

      // Map page → module
      const PAGE_MODULE = {
        'printing-v2': 'printing', 'printing': 'printing',
        'stitching-v3': 'stitching', 'stitching-v2': 'stitching', 'stitching': 'stitching',
        'delivery': 'delivery', 'delivery-v2': 'delivery',
        'verifier': 'verifier', 'verification-dashboard': 'verifier',
        'payments': 'payments', 'outstanding': 'outstanding',
        'launch-campaign': 'launch-campaign', 'execution-plan': 'execution-plan',
        'execution-entry': 'execution-entry', 'rates-master': 'rates-master',
        'master-cache': 'master-cache', 'ppt': 'ppt', 'dashboard': 'dashboard',
        'owner': 'owner', 'admin-v2': 'admin-v2', 'admin': 'admin-v2',
        'assign-operators': 'assign-operators', 'designer-uploads': 'designer-uploads',
        'design-tasks': 'design-tasks', 'campaign-status': 'campaign-status',
        'report-daily': 'report-daily', 'report-team': 'report-team',
        'report-campaign': 'report-campaign', 'reports': 'reports',
        'crm': 'crm', 'leads': 'leads', 'quotations': 'quotations',
        'pi': 'pi', 'invoices': 'invoices', 'clients': 'clients',
        'field': 'field', 'manual-upload': 'manual-upload', 'expenses': 'expenses'
      };
      const mod = PAGE_MODULE[path];
      if (!mod) return true;  // unknown page → don't block (avoid lockout)
      return hasModuleAccess(member, mod);
    },
    ...
    getDefaultHome() {
      ...
      // Field workers → separate field app (own login/session)
      if (role === 'field' || role === 'field_worker') return '/field.html';

      // Admin / manager roles → V2 Admin hub (they need the full multi-feature UI).
      // admin-v2 filters visible features by module_access.
      const ADMIN_HUB_ROLES = [
        'owner', 'admin', 'operations_manager', 'ops_manager', 'operations',
        'data_entry', 'deo', 'data_manager', 'design', 'designer',
        'sales', 'hr_sales', 'sales_admin'
      ];
      if (ADMIN_HUB_ROLES.indexOf(role) !== -1) return '/app.html#v2';

      // Operational / single-task roles → straight to THEIR own page.
      // (printing operator ko printing page, na ki admin hub jahan se woh
      //  galti se kahin aur chala jaaye.)
      const ROLE_HOME = {
        'printing':   '/printing-v2.html',
        'printer':    '/printing-v2.html',
        'stitching':  '/stitching-v3.html',
        'tailor':     '/stitching-v3.html',
        'delivery':   '/delivery.html',
        'driver':     '/delivery.html',
        'verifier':   '/verifier.html',
        'execution':  '/execution-entry.html',
        'accountant': '/payments.html'
      };

      // ⭐ OVERRIDE: agar operational role ke paas module_access se admin-v2 ke
      // modules grant kiye gaye hain (verifier ke alawa), to use admin-v2 hub
      // bhejо — taaki woh saare granted modules dekh sake (sidebar filter karega).
      // Bina iske, verifier login pe seedha verifier.html jaata aur grants bekaar.
      const ma = (member.module_access && typeof member.module_access === 'string')
        ? (function(){ try { return JSON.parse(member.module_access); } catch(e){ return {}; } })()
        : (member.module_access || {});
      // admin-v2 ke modules jo Team Access UI se grant hote (ids must match TA_SECTIONS)
      const ADMIN_V2_MODULES = ['dashboard','campaigns','members','assignments','photos',
        'reports','clientlinks','ppt','master-cache','manual-upload',
        'launch-campaign','campaign-status','assign-operators','designer-uploads',
        'execution-plan','execution-entry','rates-master','ops-dashboard',
        'printing','stitching','delivery','expenses','payments','outstanding',
        'report-daily','report-team','report-campaign','reassignment-history','archive-viewer',
        'crm','leads','quotations','pi','invoices','clients','hub','sales-hub'];
      var hasAdminV2Grant = false;
      for (var k in ma) {
        if (ma[k] === true && ADMIN_V2_MODULES.indexOf(k) !== -1) { hasAdminV2Grant = true; break; }
      }
      if (hasAdminV2Grant) return '/app.html#v2';

      if (ROLE_HOME[role]) return ROLE_HOME[role];

      // Unknown role → safe default (admin hub, which will filter to near-empty)
      return '/app.html#v2';
    },
