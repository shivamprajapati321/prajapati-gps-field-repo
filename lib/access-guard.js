// ═══════════════════════════════════════════════════════════════════════
// /lib/access-guard.js — v3.0 (with Data Filter for RLS)
// 
// NEW IN v3.0:
...
// 
// Usage in sales pages:
//   const filter = AccessGuard.getDataFilter();
//   const url = SUPABASE_URL + '/rest/v1/trial_leads?' + filter + '&select=*';
// ═══════════════════════════════════════════════════════════════════════
...
(function(window) {
  ...
  const AccessGuard = {
    ...
    getDefaultHome() {
      ...
      if (role === 'sales' || role === 'hr_sales' || role === 'sales_admin') return '/sales-hub.html';
  ...
  console.log('%c🛡️ AccessGuard v3.0 loaded (with RLS)', 'background:#0B1957;color:#D4B45F;padding:4px 10px;border-radius:4px;font-weight:bold;');
