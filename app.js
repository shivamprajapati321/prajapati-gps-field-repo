...
function openInAppCamera(key){
  ...
  navigator.mediaDevices.getUserMedia({
    ...
    v.onloadedmetadata = function(){
      ...
      // PATCH 3: Reset zoom to native default 1.0x
      var track = stream.getVideoTracks()[0];
      if (track && track.getCapabilities) {
        try {
          var caps = track.getCapabilities();
          if (caps.zoom) {
            var targetZoom = Math.max(1.0, caps.zoom.min);
            if (targetZoom > caps.zoom.max) targetZoom = caps.zoom.max;
            track.applyConstraints({ advanced: [{ zoom: targetZoom }] }).then(function() {
              camZoomState.currentZoom = targetZoom;
              console.log('[CAM] Zoom set to native default:', targetZoom + 'x (min='+caps.zoom.min+', max='+caps.zoom.max+')');
